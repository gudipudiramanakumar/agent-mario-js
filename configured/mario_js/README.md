# Mario JS — Agentic Demo Fork

> This is a modified version of the original Mario JS project, adapted for use with an AI agent control system. See modifications below, then the original documentation.

## Modifications for Agentic Use

Only two changes were made from the original source. All other files are unmodified.

### 1. `lib/game.js` — Expose live game state

Added at the end of game initialisation:
```js
window.game = { data, input };
```
Required — without this, `gameAPI.js` cannot read Mario's position, entity states, or dispatch inputs. The agent would be blind.

### 2. `lib/map/level_1-1.js` — Koopa Troopas removed

```js
koopas: [],  // Removed for demo — shell timing incompatible with API latency
```
Koopa shell physics require sub-100ms reactions. An AI agent over HTTP has ~500ms+ per action, making shell interactions unreliable. Goombas are used as the enemy type instead.

### Build Note
`assets/javascripts/bundle.js` is a webpack artifact compiled from `lib/`. Do not edit it directly. If you modify `lib/` files, rebuild with:
```bash
